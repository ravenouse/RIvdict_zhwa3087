...
###version change: import new tokenizer to retokenize
import tokenizers
...
language_list1 = ["<en>","<es>","<fr>","<it>","<ru>"]
language_list2 = ["<en>","<fr>","<ru>"]
...
class JSONDataset(Dataset):
    ...
    def __init__(self, file, tokenizer, vocab=None,freeze_vocab=False, maxlen=256):
        """
        ...
        for idx, json_dict in enumerate(self.items):
            ...
            json_dict['idx'] = idx
            if "all" in type and 'electra' not in type:
                if "train" in type:
                    json_dict['language_token'] = language_list1[idx//43608]
                    # print('train file')
                elif "dev" in type:
                    json_dict['language_token'] = language_list1[idx //6375]
                    # print('dev file')
            elif "all" in type and 'electra' in type:
                if "train" in type:
                    json_dict['language_token'] = language_list2[idx//43608]
                    # print('train file')
                elif "dev" in type:
                    json_dict['language_token'] = language_list2[idx //6375]
                    # print('dev file')
            else:
...
def get_dataloader(dataset, batch_size=200, shuffle=True):
    ...
    def do_collate(json_dicts):
        ...
        if has_gloss:
            ...
            # batch["new_gloss_tensor"] = new_gloss_tensor
        if has_vecs:
            for arch in SUPPORTED_ARCHS:
...
def retokenize(batch,dir):
    tokenizer = Tokenizer.from_file(str(dir))
    new_vocab = tokenizer.get_vocab()
    new_gloss_tensor = []
    output = tokenizer.encode_batch(batch["gloss"])
    for n in range(len(output)):
        one_gloss = [new_vocab[batch['language_token'][n]]]+ [new_vocab[BOS]] + output[n].ids + [new_vocab[EOS]]
        one_gloss_tensor = torch.tensor(one_gloss)
        new_gloss_tensor.append(one_gloss_tensor)
    new_gloss_tensor = pad_sequence(new_gloss_tensor, batch_first=True, padding_value=new_vocab[PAD]).T
    batch["new_gloss_tensor"] = new_gloss_tensor
