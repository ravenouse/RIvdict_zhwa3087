...
class RevdictModel(nn.Module):
    ...
    def __init__(
        ...
        encoder_layer_cut2 = transformers_cut.TransformerEncoderLayer_cut(
            d_model=d_model, nhead=n_head, dropout=dropout, dim_feedforward=d_model * 2
        )
        self.transformer_encoder = transformers_cut.TransformerEncoder_cut(
            encoder_layer, num_layers=n_layers,encoder_layer_cut=encoder_layer_cut2
        )
        self.dropout = nn.Dropout(p=dropout)
