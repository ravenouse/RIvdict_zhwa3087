...
class RevdictModel(nn.Module):
    """A LSTM Rnn version for Reverse dictionary Modeling."""
    ...
    def __init__(
        ...
        self.embedding = nn.Embedding(len(vocab), d_model, padding_idx=self.padding_idx)
        self.lstm = nn.LSTM(d_model,hidden_dim,n_layers,dropout=dropout,batch_first=False,bidirectional=False)
    ...
    def init_hidden(self,batch_size):
        return (torch.ones(self.n_layers,batch_size,self.hidden_dim).float().to(torch.device('cuda')),
                torch.ones(self.n_layers,batch_size,self.hidden_dim).float().to(torch.device('cuda')))
